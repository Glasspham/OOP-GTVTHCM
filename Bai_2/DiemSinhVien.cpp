...
DiemHocVien::DiemHocVien(std::string hoTen, int namSinh, float diemMon1, float diemMon2, float diemMon3)
    : hoTen(hoTen), namSinh(namSinh), diemMon1(diemMon1), diemMon2(diemMon2), diemMon3(diemMon3) {}

DiemHocVien::~DiemHocVien() {}

std::string DiemHocVien::getHoTen() const { return hoTen; }
...
std::vector<std::string> DiemHocVien::getMonThiLai() const {
    std::vector<std::string> monThiLai;
    if (diemMon1 < 5) {
        monThiLai.push_back("Mon 1");
    }
    if (diemMon2 < 5) {
        monThiLai.push_back("Mon 2");
    }
    if (diemMon3 < 5) {
        monThiLai.push_back("Mon 3");
    }
    return monThiLai;
}

std::ostream& operator<<(std::ostream& os, const DiemHocVien& hocVien) {
    os << "Ho ten: " << hocVien.hoTen << ", Nam sinh: " << hocVien.namSinh << std::endl;
    os << "Diem mon 1: " << hocVien.diemMon1 << ", Diem mon 2: " << hocVien.diemMon2 << ", Diem mon 3: " << hocVien.diemMon3 << std::endl;
    return os;
}